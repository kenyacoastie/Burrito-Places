...
export default class PlaceMap extends Component {
  ...
  getBurritoData() {
    //axios request to get all nearby burrito spots, setting it to
    //annotations to render on the map
    axios.get(`https://maps.googleapis.com/maps/api/place/nearbysearch/json?location=${this.state.currentRegion.latitude},${this.state.currentRegion.longitude}&radius=5000&types=food&keyword=burrito&key=AIzaSyDNhxQlwToYhiZuucMmLly6m_YTz1P0KOQ`)
    .then((response) => {
      console.log(response.data.results[0]);
      response.data.results.forEach((burrito) => {
      this.props.onAddPlace({
          title: burrito.name,
          subtitle: 'Burrito Rating: ' + burrito.rating,
